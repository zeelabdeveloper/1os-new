...
const fetchRecruiterStats = async (recruiterId, dateRange) => {
  try {
    ...
    const response = await axios.get("/api/v1/hiring/recruiter/stats", { params });
    return response.data.data;
  } catch (error) {
    toast.error("Failed to fetch recruiter stats");
    throw error;
  }
};

function RecruiterStats({recruiterId}) {
  ...
  const {
    ...
    queryKey: ["recruiterStats", recruiterId, dateRange],
    queryFn: () => fetchRecruiterStats(recruiterId, dateRange),
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-64">
        <Spin size="large" tip="Loading recruiter statistics..." />
      </div>
    );
  }
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <Alert
          ...
          description="Failed to load recruiter statistics. Please try again later."
          type="error"
          showIcon
        />
      </div>
  ...
  return (
    <div className="p-4 h-[92vh] overflow-y-auto">
      <Title level={2} className="mb-6">
        <BarChartOutlined /> Recruiter Hiring Dashboard
      </Title>
      ...
      {data?.recruiterInfo && (
        <Card className="mb-6">
          <div className="flex items-center">
            <Avatar 
              size={64} 
              src={data.recruiterInfo.profilePicture} 
              icon={<UserOutlined />}
              className="mr-4"
            />
            <div>
              <Title level={4} className="mb-1">
                {data.recruiterInfo.firstName} {data.recruiterInfo.lastName}
              </Title>
              <Text type="secondary">{data.recruiterInfo.email}</Text>
              <div className="mt-2">
                <Tag color="blue">Recruiter</Tag>
                {data.recruiterInfo.phone && (
