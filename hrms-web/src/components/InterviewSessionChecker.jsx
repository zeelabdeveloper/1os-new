...
const InterviewSessionManager = ({user}) => {
  ...
  const handleSubmitFeedback = (sessionId) => {
   ...
   const hasNullAnswer = answer.some(ans => ans?.answer === null || ans?.answer === undefined);
                ...
                <div className="flex flex-col md:flex-row justify-between gap-4">
                  ...
                  <div className="flex-1 flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <Text type="secondary" className="block">
                        {session.interviewRoundId?.passingScore || "NA" }% • Passing Score{" "}
...
{/* {session?.interviewRoundId?.questions && Array.isArray(session?.interviewRoundId?.questions) && (
  <>
    <Divider orientation="left">Interview Questions</Divider>
    {session.interviewRoundId.questions.map((question, index) => (
      <Form.Item
        key={question._id} 
        label={`${index + 1}. ${question.questionText} (Weightage: ${question.weightage}%)`}
        ...
        <Radio.Group 
          onChange={(e) => {
            setAnswer(prev => {
              ...
              const existingIndex = prev.findIndex(item => item.questionId === question._id);
              ...
              if (existingIndex >= 0) {
                ...
                return [
                  ...prev,
                  {
                    ...
                    questionText: question.questionText,
                    weightage: question.weightage,
                    expectedAnswer: question.expectedAnswer // Keep original expected answer
                  }
                ];
              }
            });
...
{session?.interviewRoundId?.questions && Array.isArray(session?.interviewRoundId?.questions) && (
  <>
    ...
    {session?.interviewRoundId?.questions.map((question, index) => {
      ...
      const existingAnswer = answer.find(a => a.questionId === question._id);
                      ...
                      <Panel
                        header={
                          <Text strong className="flex items-center">
                            ...
                            Rate Skills (
                            {
                              Object.keys(ratings).filter((k) => ratings[k] > 0)
                                .length
                            }
                            /{SKILLS_LIST.length})
                          </Text>
